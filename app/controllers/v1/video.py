...
def _sanitize_upload_filename(filename: str, request_id: str) -> str:
    # Browsers or clients sometimes attach directory information, and may even include
    # path traversal fragments like "../". Strip everything except the bare filename
    # to prevent the upload endpoint from writing files outside the target directory.
...
def _resolve_path_within_directory(base_dir: str, unsafe_path: str, request_id: str) -> str:
    # Normalize the user-supplied relative path and require that the result still
    # resolves inside the given directory. This blocks attempts to escape the task
    # directory via "../" or absolute paths to read arbitrary files.
...
def get_video_materials_list(request: Request):
    ...
    # Filesystem enumeration order is not stable; returning results directly would
    # cause "sequential concatenation" to behave inconsistently across machines or
    # over time. Sort by filename so the server returns a predictable order.
...
def upload_video_material_file(request: Request, file: UploadFile = File(...)):
    ...
    # Validate using the lowercased extension so files with uppercase suffixes
    # like .MOV are accepted.
