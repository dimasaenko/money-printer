...
if "local_video_materials" not in st.session_state:
    # Remember the user's most recently persisted local materials, so the list is not lost when only the script is modified and re-generated.
...
# Create a top bar containing the title and the language selector
...
def init_log():
    ...
    def format_record(record):
        ...
        # Update the file path on the record
...
if not config.app.get("hide_config", False):
    with st.expander(tr("Basic Settings"), expanded=False):
        ...
        with left_config_panel:
            ...
            # Whether to disable the log display
        ...
        with middle_config_panel:
            ...
            if llm_provider == "ollama":
                ...
                with llm_helper:
                    tips = """
                            ...
                            - **API Key**: Any value works, for example `123`
                            - **Base Url**: Usually `http://localhost:11434/v1`
                                - If `MoneyPrinterTurbo` and `Ollama` are **not on the same machine**, use the IP address of the `Ollama` host
                                - If `MoneyPrinterTurbo` runs inside `Docker`, we recommend `http://host.docker.internal:11434/v1`
                            - **Model Name**: Run `ollama list` to see available models, for example `qwen:7b`
                            """
            ...
            if llm_provider == "openai":
                ...
                with llm_helper:
                    tips = """
                            ...
                            > A VPN with global traffic mode may be required in some regions
                            - **API Key**: [Apply on the official site](https://platform.openai.com/api-keys)
                            - **Base Url**: Leave blank for the official OpenAI API. For OpenAI-compatible providers (e.g. OpenRouter), fill in the compatible endpoint URL
                            - **Model Name**: Enter a model you **have access to**. For compatible providers, use a model ID supported by that platform
                            """
            ...
            if llm_provider == "moonshot":
                ...
                with llm_helper:
                    tips = """
                            ...
                            - **API Key**: [Apply on the official site](https://platform.moonshot.cn/console/api-keys)
                            - **Base Url**: Fixed at `https://api.moonshot.cn/v1`
                            - **Model Name**: For example `moonshot-v1-8k`. [View the model list](https://platform.moonshot.cn/docs/intro#%E6%A8%A1%E5%9E%8B%E5%88%97%E8%A1%A8)
                            """
            if llm_provider == "oneapi":
                ...
                with llm_helper:
                    tips = """
                        ...
                        - **Model Name**: Enter the model name you want to use, for example `claude-3-5-sonnet-20240620`
                        """
            ...
            if llm_provider == "qwen":
                ...
                with llm_helper:
                    tips = """
                            ...
                            - **API Key**: [Apply on the official site](https://dashscope.console.aliyun.com/apiKey)
                            - **Base Url**: Leave blank
                            - **Model Name**: For example `qwen-max`. [View the model list](https://help.aliyun.com/zh/dashscope/developer-reference/model-introduction#3ef6d0bcf91wy)
                            """
            ...
            if llm_provider == "g4f":
                ...
                with llm_helper:
                    tips = """
                            ...
                            > [Open-source GitHub project](https://github.com/xtekky/gpt4free) — free access to GPT models, but **stability is limited**
                            - **API Key**: Any value works, for example `123`
                            - **Base Url**: Leave blank
                            - **Model Name**: For example `gpt-3.5-turbo`. [View the model list](https://github.com/xtekky/gpt4free/blob/main/g4f/models.py#L308)
                            """
            if llm_provider == "azure":
                with llm_helper:
                    tips = """
                            ##### Azure Configuration
                            > [See how to deploy a model](https://learn.microsoft.com/en-us/azure/ai-services/openai/how-to/create-resource)
                            - **API Key**: [Create in the Azure portal](https://portal.azure.com/#view/Microsoft_Azure_ProjectOxford/CognitiveServicesHub/~/OpenAI)
                            - **Base Url**: Leave blank
            ...
            if llm_provider == "gemini":
                ...
                with llm_helper:
                    tips = """
                            ...
                            > A VPN with global traffic mode may be required in some regions
                            - **API Key**: [Apply on the official site](https://ai.google.dev/)
                            - **Base Url**: Leave blank
            ...
            if llm_provider == "deepseek":
                ...
                with llm_helper:
                    tips = """
                            ...
                            - **API Key**: [Apply on the official site](https://platform.deepseek.com/api_keys)
            ...
            if llm_provider == "modelscope":
                ...
                with llm_helper:
                    tips = """
                            ...
                            - **API Key**: [Apply on the official site](https://modelscope.cn/docs/model-service/API-Inference/intro)
                            - **Base Url**: Fixed at `https://api-inference.modelscope.cn/v1/`
                            - **Model Name**: For example `Qwen/Qwen3-32B`. [View the model list](https://modelscope.cn/models?filter=inference_type&page=1)
                            """

            if llm_provider == "ernie":
                with llm_helper:
                    tips = """
                            ##### Baidu ERNIE (文心一言) Configuration
                            - **API Key**: [Apply on the official site](https://console.bce.baidu.com/qianfan/ais/console/applicationConsole/application)
                            - **Secret Key**: [Apply on the official site](https://console.bce.baidu.com/qianfan/ais/console/applicationConsole/application)
                            - **Base Url**: Enter the **request URL**. [View the docs](https://cloud.baidu.com/doc/WENXINWORKSHOP/s/jlil56u11#%E8%AF%B7%E6%B1%82%E8%AF%B4%E6%98%8E)
                            """
            ...
            if tips and config.ui["language"] == "zh":
                st.warning(
                    "Users in mainland China are encouraged to use **DeepSeek** or **Moonshot** as the LLM provider\n- Directly accessible from within China, no VPN required\n- Free credits on signup, usually enough to get started"
                )
                st.info(tips)
...
with middle_panel:
    with st.container(border=True):
        ...
        if params.video_source == "local":
            # Streamlit's file-type validation is case-sensitive on extensions, so allow both lowercase and uppercase forms.
    ...
    with st.container(border=True):
        ...
        # Add the TTS server selector dropdown
        tts_servers = [
        ...
        # Get the saved TTS server, defaulting to v1
        ...
        # Fetch the voice list for the selected TTS server
        filtered_voices = []

        if selected_tts_server == "siliconflow":
            # Fetch the SiliconFlow voice list
            filtered_voices = voice.get_siliconflow_voices()
        elif selected_tts_server == "gemini-tts":
            # Fetch the Gemini TTS voice list
            filtered_voices = voice.get_gemini_voices()
        else:
            # Fetch the Azure voice list
        ...
        # Check whether the saved voice is present in the current filtered voice list
        if saved_voice_name in friendly_names:
            saved_voice_name_index = list(friendly_names.keys()).index(saved_voice_name)
        else:
            # Otherwise, pick a default voice based on the current UI language
            for i, v in enumerate(filtered_voices):
                if v.lower().startswith(st.session_state["ui_language"].lower()):
                    saved_voice_name_index = i
                    break

        # If no matching voice was found, fall back to the first voice
        ...
        if friendly_names:
            ...
            # If no voices are available, show a notice
            st.warning(
                tr(
        ...
        # Only show the preview button when voices are available
        ...
        # When the V2 server is selected or the voice is a V2 voice, show the Speech Region and API key inputs
        ...
        # When SiliconFlow is selected, show the API key input and help text
        if selected_tts_server == "siliconflow" or (
            ...
            # Show the SiliconFlow help text
            st.info(
...
if start_button:
    ...
    if uploaded_files:
        ...
        # On each re-upload, use the currently selected files as the source of truth to avoid repeatedly appending old materials.
        ...
        # Persist uploaded and saved local video materials into the session so they can be reused when only the script is modified later.
        st.session_state["local_video_materials"] = persisted_local_materials
    elif params.video_source == "local" and st.session_state["local_video_materials"]:
        # When the user has not re-uploaded files, reuse the most recent local materials already persisted to disk.
        params.video_materials = []
